...
路由重定向可以通过使用 `<Routes>` 组件和 `<Route>` 组件的 `element` 属性来实现。以下是一个使用重定向的示例：
...
在上面的示例中，`<Route>` 组件的 `path` 属性是 `/user/:id`，其中的 `:id` 表示一个动态的路由参数。当用户访问 `/user/123` 这个路径时，`User` 组件会被渲染，并通过 `useParams()` 钩子获取到动态路由参数 `{ id: '123' }`。在 `User` 组件中，我们可以根据需要使用这个参数。
...
以下是一个示例，演示如何在父组件中使用 `Outlet` 渲染子组件：
